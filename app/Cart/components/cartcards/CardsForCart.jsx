...
export function BookingCard() {
  ...
  const handleRemoveFromCart = async (id) => {
    const token = localStorage.getItem("token")
    try {
      const response = await fetch(`/api/cart/remove-from-cart?id=${id}`, {
        method: "DELETE",
        headers: {
          Authorization: token,
        },
      ...
      if (response.ok) {
        ...
        window.location.reload();
        // You may want to perform additional actions after removing the item
      } else {
        // Handle error response
  ...
  if (cartItems.length > 0) {
    return (
      <>
        ...
        <div className="px-12 py-4 flex flex-wrap justify-evenly gap-2">
          {cartItems.map((item) => (
            <Card
              className="lg:w-1/4 md:w-2/3 w-full h-[33rem] shadow-lg mb-8"
              key={item.id}
            >
              <CardHeader
                floated={false}
                color="blue-gray"
                className="object-contain w-76 h-76"
              >
                <img src={`${item.image_url}`} alt="ui/ux review check" />
              ...
              <CardBody className="items-end">
                ...
                <div className="group mt-8 inline-flex flex-wrap items-center gap-3">
                  ...
                  <Tooltip content="Remove From Cart">
                    <span
                      className="cursor-pointer rounded-full border border-gray-900/5 bg-gray-900/5 p-3 text-gray-900 transition-colors hover:border-gray-900/10 hover:bg-red-600 hover:!opacity-100 group-hover:opacity-70"
                  ...
                  <Tooltip content="View Service">
                    <span className="cursor-pointer rounded-full border border-gray-900/5 bg-gray-900/5 p-3 hover:text-white text-gray-900 transition-colors hover:border-gray-900/10  hover:bg-blue-500 hover:!opacity-100 group-hover:opacity-70">
                      <svg
                        ...
                        className="w-6 h-6"
                        onClick={() => {
                          route.push(`/ProductMain/${item.service_id}`);
                        }}
                      >
                        <path
                          strokeLinecap="round"
